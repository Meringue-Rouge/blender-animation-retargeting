...
def get_action_fcurves(action):
    """Helper to get all F-Curves from a layered action in Blender 4.2+."""
    fcurves = []
    if action:
...
def get_keyframes(obj):
    ...
    if anim is not None and anim.action is not None:
        for fcu in get_action_fcurves(anim.action):
...
def transfer_anim(ctx):
    ...
    if not keyframes:
        info('No keyframes found in source; skipping bake')
        return
    ...
    # Setup layered action structure for Blender 5.0+
    if not target_action.layers:
        layer = target_action.layers.new(name="BakeLayer")
        strip = layer.strips.new()  # Type defaults to 'KEYFRAME'; no start/end as strips are infinite

    # Create and assign slot
    slot = target_action.slots.new(id_type='OBJECT', name=ctx.target.name)

    anim = ctx.target.animation_data
    if not anim:
        anim = ctx.target.animation_data.create()
    ...
    if ctx.setting_bake_linear:
        for fc in get_action_fcurves(target_action):
